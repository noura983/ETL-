...
from
    ...
    INNER JOIN TPCDS.RAW.DATE_DIM d ON cs.CS_SOLD_DATE_SK = d.d_date_sk
group by
    1,
    2
order by
...
from
    ...
    INNER JOIN TPCDS.RAW.DATE_DIM d ON inv.inv_date_sk = d.d_date_sk
group by
    1,
    2
order by
...
from
    ...
    INNER JOIN TPCDS.RAW.DATE_DIM d ON ws.WS_SOLD_DATE_SK = d.d_date_sk
group by
    1,
    2
order by
...
from
    ...
    INNER JOIN TPCDS.RAW.DATE_DIM as dim ON dim.d_date_sk = fact.cs_sold_date_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.TIME_DIM as dim ON dim.t_time_sk = fact.cs_sold_time_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.DATE_DIM as dim ON dim.d_date_sk = fact.cs_ship_date_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.CUSTOMER as dim ON dim.c_customer_sk = fact.cs_bill_customer_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.CUSTOMER_DEMOGRAPHICS as dim ON dim.cd_demo_sk = fact.cs_bill_cdemo_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.HOUSEHOLD_DEMOGRAPHICS as dim ON dim.hd_demo_sk = fact.cs_bill_hdemo_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.CUSTOMER_ADDRESS as dim ON dim.ca_address_sk = fact.cs_bill_addr_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.CUSTOMER as dim ON dim.c_customer_sk = fact.cs_ship_customer_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.HOUSEHOLD_DEMOGRAPHICS as dim ON dim.hd_demo_sk = fact.cs_ship_hdemo_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.CALL_CENTER as dim ON dim.cc_call_center_sk = fact.cs_call_center_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.CATALOG_PAGE as dim ON dim.cp_catalog_page_sk = fact.cs_catalog_page_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.SHIP_MODE as dim ON dim.sm_ship_mode_sk = fact.cs_catalog_page_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.WAREHOUSE as dim ON dim.w_warehouse_sk = fact.cs_warehouse_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.ITEM as dim ON dim.i_item_sk = fact.cs_item_sk
limit
    5;
...
from
    TPCDS.RAW.CATALOG_SALES as fact
    INNER JOIN TPCDS.RAW.PROMOTION as dim ON dim.p_promo_sk = fact.cs_promo_sk
limit
    5;
----- Web Sales
select
...
from
    ...
    INNER JOIN TPCDS.RAW.WEB_PAGE as dim ON dim.wp_web_page_sk = fact.ws_web_page_sk
limit
    5;
...
from
    TPCDS.RAW.WEB_SALES as fact
    INNER JOIN TPCDS.RAW.WEB_SITE as dim ON dim.web_site_sk = fact.ws_web_site_sk
limit
    5;
...
select
    DISTINCT date.cal_dt,
    date.wk_num
from
    TPCDS.RAW.DATE_DIM as date
    INNER JOIN TPCDS.RAW.INVENTORY as inv ON date.d_date_sk = inv.inv_date_sk
    AND inv_item_sk = 1
ORDER BY
    1,
    2;
-- inventory table records it every week
...
from
    ...
    INNER JOIN TPCDS.RAW.DATE_DIM as date ON date.d_date_sk = ws.ws_sold_date_sk
    INNER JOIN TPCDS.RAW.ITEM as item ON item.i_item_sk = ws.ws_item_sk
    AND ws_item_sk = 1 -- i_item_desc = 'Powers will not get influences. Electoral ports should show low, annual chains. Now young visitors may pose now however final pages. Bitterly right children suit increasing, leading el'
ORDER BY
    1,
    2;
-- Erratic sales with every day as a lowest common factor
...
from
    ...
    INNER JOIN TPCDS.RAW.DATE_DIM as date ON date.d_date_sk = cs.cs_sold_date_sk
    INNER JOIN TPCDS.RAW.ITEM as item ON item.i_item_sk = cs.cs_item_sk
    AND cs_item_sk = 1
ORDER BY
    1,
    2;
